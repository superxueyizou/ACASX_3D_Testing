...
public class ACASX3D_SingleThreat 
{
	...
	private int coordinationSense;//-1-->do not climb; 0-->no restriction; 1-->do not descend
	...
	public void update(Double3D ownshipLoc, Double3D ownshipVel, Double3D intruderLoc, Double3D intruderVel, int lastRA, int coordinationSense) 
	{
		...
		this.lastRA = lastRA;
		this.coordinationSense = coordinationSense;
	}
	...
	public int execute()
	{
		...
		{
			return -1; // meaning: there is no need to use ACAS X RAs, different from 0 (CoC)				
		}		
		...
		for (Entry<Integer,Double> entry : entrySet) 
		{
			...
			if(senseCompatible(actionCode, coordinationSense) && value-maxQValue>=0.0001)
			{
				maxQValue=value;
...
//		System.out.println(maxQValue);
	...
	public Map<Integer, Double> calculateEntryTimeDistributionDTMC(Double3D ownshipLoc, Double3D ownshipVel, Double3D intruderLoc, Double3D intruderVel)
	{
		...
		assert (Math.abs(theta)<= DTMC.UPPER_THETA);
...
//		state.information=String.format( "(%.1f, %.1f, %.1f, %d)",h,oVy,iVy,ra);
	...
	public boolean senseCompatible(int actionCode, int coordinationSense)
	{
		int m=actionCode%2;
		if(m==0 && coordinationSense<=0)
			return true;
		else if(m==1 && coordinationSense>=0)
			return true;
		else
			return false;
	}
