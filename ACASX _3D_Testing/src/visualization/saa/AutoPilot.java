...
public class AutoPilot implements Steppable
{
	...
	public static final boolean RUNMODEL=false;
	...
	public void step(SimState simState) 
	{
		...
		{	
			...
			{
				if(RUNMODEL)
					hostUAS.setApNextWp(executeActionCode_Model(acasxActionCode));
				else
			...
			{
				if(RUNMODEL)
					hostUAS.setApNextWp(executeWhiteNoise_Model());
				else
	...
	private Waypoint executeActionCode_Model(int actionCode)
	{	
		assert (actionCode>=0);
	...
	public Waypoint executeWhiteNoise_Model()
	{		
		double vx=hostUAS.getVel().x;
		double vy=hostUAS.getVel().y;
		double vz=hostUAS.getVel().z;
